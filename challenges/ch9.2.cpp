...
int main() {
  ...
  // left_wall
  Plane left_wall =
      Plane(translation_matrix(-5.0, 0, 0) * rotation_z_matrix(M_PI / 2),
            Material(Color(1, 0, 0),  // color
                     0.1,             // ambient
                     0.5,             // diffuse
                     0,               // specular
                     0                // shininess
  ...
  // left_mid_wall
  Plane left_mid_wall =
      Plane(translation_matrix(0, 0, 5.77) * rotation_y_matrix(-M_PI / 6) *
                rotation_x_matrix(M_PI / 2),
            Material(Color(0.66, 0, 0.34),  // color
                     0.1,                   // ambient
                     0.5,                   // diffuse
                     0,                     // specular
                     0                      // shininess
  ...
  Plane right_mid_wall =
      Plane(translation_matrix(0, 0, 5.77) * rotation_y_matrix(M_PI / 6) *
                rotation_x_matrix(M_PI / 2),
            Material(Color(0.34, 0, 0.66),  // color
                     0.1,                   // ambient
                     0.5,                   // diffuse
                     0,                     // specular
                     0                      // shininess
  ...
  // roof
  Plane roof =
      Plane(translation_matrix(0, 5, 0), Material(Color(1, 0.9, 0.9),  // color
                                                  0.1,  // ambient
                                                  0.5,  // diffuse
                                                  0,    // specular
                                                  0     // shininess
  ...
  Sphere middle =
      Sphere(scaling_matrix(2, 1.5, 2), Material(Color(0.1, 1, 0.5),  // color
                                                 0.1,                 // ambient
                                                 0.7,                 // diffuse
                                                 0.3,   // specular
                                                 200.0  // shininess
  ...
  camera.transform =
      view_transform(point(0, 3, -5), point(0, 1.3, 0), vector(0, 1, 0));

  // Create world
  World w(std::vector<Shape *>{&floor, &left_wall, &left_mid_wall,
                               &right_mid_wall, &right_wall, &roof, &middle},
          light);
