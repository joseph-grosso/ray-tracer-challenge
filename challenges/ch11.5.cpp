...
int main() {
  ...
  CheckersPattern wall_pat =
      CheckersPattern(&off_white, &off_black, identity_matrix(4));
  ...
  // glass_sphere
  Sphere s1 = glass_sphere(translation_matrix(0, 1, 0));
  ...
  // glass_sphere
  Sphere s2 =
      Sphere(translation_matrix(0, 1, 0) * scaling_matrix(0.5, 0.5, 0.5));
  ...
  // Create world
  World w(
      std::vector<Shape *>{&floor, &left_mid_wall, &right_mid_wall, &s1, &s2},
      light);
