...
int main() {
    ConicalRingPattern black_stripe = ConicalRingPattern(Color(1, 1, 1), Color(0.1, 0.1, 0.1));
    CheckersPattern maroon = CheckersPattern(Color(0.9, 0.9, 0.9), Color(0.5, 0, 0), rotation_z_matrix(M_PI / 8) * rotation_y_matrix(-M_PI / 6) * (0.3, 0.3, 0.3) * translation_matrix(0.2, 0, 0) * 0.75);
    GradientPattern teal = GradientPattern(Color(1, 1, 1), Color(0, 0.5, 0.5), rotation_y_matrix(- 2 * M_PI / 3) * translation_matrix(0.7, 0, 0) * scaling_matrix(2, 2, 2));
    ConicalRingPattern orange = ConicalRingPattern(Color(1, 1, 1), Color(1, 215.0/255.0, 0), scaling_matrix(0.13, 0.13, 0.13F) * rotation_z_matrix(M_PI / 10) * translation_matrix(0.2, 0, 0));
